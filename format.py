...
# This gives better results, but seems wrong since graph node properties are added randomly at the end
def init_graph_v1(graphs, number_of_examples, number_of_classes, noise, board_width, data):
...
def init_graph(graphs, number_of_examples, number_of_classes, noise, board_width, data):
    ...
    for graph_id in range(number_of_examples):
        ...
        for node_id in range(graphs.number_of_graph_nodes[graph_id]):
            ...
            try:
                # symbol = Y_train[graph_id]
                print("Symbol")
                print(data[graph_id]['board'][node_id])

                symbol = data[graph_id]['board'][node_id]  # 0 if data[graph_id]['board'][node_id] == 'B' else 1
